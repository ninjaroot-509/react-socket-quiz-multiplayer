...
function GameCountdown({ number }) {
    return ( <
        ...
        starting game in
        <
        div className = "text-6xl font-black text-gray-900 text-center" > { number } <
        /div> <
        /div> <
        /div>
    );
...
export class App extends Component {
    ...
    render() {
        if (this.state.appState === "init") {
            return ( <
                JoinPage joinGame = { this.joinGame }
        ...
        if (this.state.appState === "join") {
            return ( <
                InitPage gameID = { this.state.gameID }
        ...
        return ( <
            GamePage currentQuestion = { this.state.currentQuestion }
            ...
            setUserAnswer = {
                (idx) => this.setState({ userAnswer: idx }) } >
            < /GamePage>
        );
    }
}
